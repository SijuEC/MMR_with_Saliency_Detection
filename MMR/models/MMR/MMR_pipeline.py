...
from utils import compute_pixelwise_retrieval_metrics, compute_pro, save_image, save_video_segmentations, sal_check
    ...
    def fit(self, individual_dataloader):
        ...
        for epoch in range(self.cfg.TRAIN_SETUPS.epochs):
            ...
            for image in individual_dataloader:
                ...
                reverse_features = self.mmr_model(image, mask_ratio=self.cfg.TRAIN.MMR.finetune_mask_ratio, sal_region=sal_region, inference=False)  # bn(inputs))
        ...
        with torch.no_grad():
            for image in test_dataloader:
                ...
                reverse_features = self.mmr_model(image,
                                                  mask_ratio=self.cfg.TRAIN.MMR.test_mask_ratio, inference=True)
                multi_scale_reverse_features = [reverse_features[key]
                                                for key in self.cfg.TRAIN.MMR.layers_to_extract_from]
